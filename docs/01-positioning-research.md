...
3. **Price is hidden in the FAQ.** _Update 12 Sep: the decision is to withhold pricing from the site entirely and share it on the call. The anchor problem still applies, so the site anchors against the cost of the alternatives (a GTM engineer, a partner retainer, an AI-native CRM migration) rather than against a published number._
...
- **Proof to lead with:** field completion 31% → ~100%; follow-up sent in under 5 minutes; 92–97% accuracy with "empty over wrong"; live in under a day; per-AE pricing shared on the call, with every other seat free.
