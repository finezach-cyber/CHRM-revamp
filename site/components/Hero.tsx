// 2nd Closer marketing — Hero. The promise on the left; the proof (what happens after a call) on the right.
import Link from "next/link";
import AfterTheCall from "./AfterTheCall";
import { BOOKING_URL, BETA } from "@/lib/site";

export default function Hero() {
  return (
    <section className="m-hero m-hero--split" id="top" data-screen-label="Hero">
      <div className="m-hero__grid">
        <div className="m-hero__copy">
          <div className="m-hero__eyebrow">The execution layer for HubSpot &amp; Pipedrive</div>
          <h1 className="m-hero__tagline">
            Silent on the call.
            <br />
            <em>Everything after it, done.</em>
          </h1>
          <p className="m-hero__blurb">
            2nd Closer is the AI account executive that works your deals after every call:
            it updates HubSpot or Pipedrive, writes and sends the follow-up, works the buying
            committee on LinkedIn, enriches every contact, and flags the deals that are slipping.
            It never speaks on the call and never negotiates. That part is still yours.
          </p>
          <p className="m-hero__stage">Built for seed and Series A teams with 1–50 closers. <Link href="/startups">Why founders start here &rarr;</Link></p>
          <div className="m-hero__ctas">
            <a href={BOOKING_URL} className="btn btn--primary">{BETA.cta}</a>
            <Link href="/features" className="btn btn--ghost">See how it works</Link>
          </div>
          <p className="m-hero__fine">
            {`${BETA.short} · HubSpot & Pipedrive · no migration, no engineer · live in a day`}
          </p>
        </div>
        <div className="m-hero__proof">
          <AfterTheCall />
        </div>
      </div>

      <div className="m-hero__connectors">
        <span className="m-hero__connectors-label">Works with</span>
        <ul>
          <li>HubSpot</li>
          <li>Pipedrive</li>
          <li>Google &amp; Microsoft calendar</li>
          <li>Gmail &amp; Outlook</li>
          <li>LinkedIn</li>
          <li>Apollo</li>
          <li>Fireflies, Fathom, Otter</li>
        </ul>
      </div>
    </section>
  );
}
