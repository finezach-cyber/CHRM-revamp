...
// CHRM marketing — Product feature page data
...
export const PRODUCT_PAGES: PageData[] = [
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    prose: [
      'Every CRM tells you a deal lost. CHRM tells you a deal is losing. The live risks feed is the screen managers open on Monday morning instead of the pipeline report — because by the time the pipeline report is wrong, the deal is gone.',
      'CHRM continuously scans every conversation across five risk categories. Missing decision-maker. Budget unconfirmed past stage three. Competitive threat introduced and left unanswered. Timeline slipping a quarter at a time. Champion gone quiet for fourteen days. The thresholds are yours; these are the defaults.',
      ...
      'CHRM&rsquo;s risk surface is real-time, not weekly. The moment a transcript is processed, the risks against that deal are updated. The forecast call stops being an archaeology dig and starts being a strategy meeting.',
    ],
    bullets: [
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    sub: 'Configure the structure, define the fields, set the tone. CHRM fills them in and shows you the exact line every value came from.',
    meta: [
    ...
    prose: [
      'Most AI summaries collapse a sixty-minute call into a paragraph nobody trusts. CHRM&rsquo;s engagement summaries are built differently: you decide the structure, you decide the fields, you decide the tone. CHRM does the writing — and shows you the source for every value.',
      'A summary in CHRM is not free-form text. It is a structured record with the fields your team needs in the order your team needs them. Champion. Budget signal. Timeline. Decision criteria. Top objection. Next step. Whatever your playbook actually depends on.',
      ...
      'When information is discussed clearly, CHRM produces near-perfect outputs. When information is ambiguous or wasn&rsquo;t covered, CHRM leaves the field empty rather than guess. Empty over wrong is a rule, not a polite afterthought.',
    ],
    bullets: [
      ...
      ['Empty over wrong', 'When information is ambiguous or undiscussed, CHRM leaves the field empty.'],
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    prose: [
      ...
      'CHRM solves it by removing the human from the loop entirely. Every conversation — call, email, LinkedIn touch — is structured against your CRM schema and written back to HubSpot or Pipedrive in real time. The fields fill themselves.',
      { h: 'Your schema. Not ours.' },
      'CHRM reads your existing CRM fields, including every custom field you&rsquo;ve built. Champion strength. Budget signal. Renewal lever. Whatever&rsquo;s already in your HubSpot or Pipedrive setup, CHRM populates. No template imposed. No re-architecture asked for.',
    ...
    bullets: [
      ['Your custom fields', 'Every field you\u2019ve built in HubSpot or Pipedrive. CHRM reads your schema, doesn\u2019t impose one.'],
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    prose: [
      'Follow-ups are the moment where deals quietly die. The call ends, the rep gets pulled into the next thing, and by the time the email goes out it&rsquo;s two days late and generic. CHRM closes that gap.',
    ...
    bullets: [
      ...
      ['Multi-step sequences', 'For deals that need a sequence, CHRM builds one across the next two weeks.'],
    ],
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    prose: [
      'Most sequence tools run a fixed playbook regardless of what happens in the deal. They send step three on day five whether or not the prospect replied to step two with a critical objection. CHRM&rsquo;s sequences are aware of the deal.',
      'Every step can condition on what CHRM knows: did the prospect reply, was a risk introduced, did the stage advance. Sequences pause when a real conversation is in flight and resume when it isn&rsquo;t.',
      { h: 'Two scopes.' },
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    prose: [
      ...
      'When a name surfaces mid-call — "we&rsquo;ll need to loop in Maria from security" — CHRM picks it up, proposes the role, and adds them to the map. The first time you hear about Maria isn&rsquo;t the email three weeks later when she vetoes the deal.',
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    prose: [
      'Most outreach tools stop at the rep&rsquo;s outbound. CHRM goes further. The moment a buying committee member is identified in any conversation, CHRM opens a LinkedIn connection and runs a warm-up sequence with them.',
      'The closer isn&rsquo;t doing the work. CHRM is. By the time the deal needs Maria from security to weigh in, Maria already has a relationship with someone on your team via LinkedIn. Relationships compound; this is the layer that builds them while you&rsquo;re sleeping.',
      { h: 'Safe by construction.' },
      'LinkedIn outreach is rule-bound. CHRM enforces daily action limits per account, slow warm-up cadences for new accounts, and platform-safe behaviour patterns. The whole framework is built so you scale outreach without burning the account.',
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    sub: 'Register your competitors. CHRM detects every mention in every conversation, scores threat per deal, and proposes counter-positioning grounded in the call.',
    meta: [
    ...
    prose: [
      ...
      'CHRM detects competitor mentions in every conversation. When a name shows up — "we&rsquo;re also looking at Clari" — CHRM tags the deal with a threat level and surfaces the specific positioning that addresses what the prospect actually said about the competitor, not a generic comparison sheet.',
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    sub: 'Classify every deal into your industry segments, with confidence scores and the signals CHRM used, for nurturing, ICP scoring, and segment reporting.',
    meta: [
    ...
    prose: [
      ...
      'CHRM classifies every deal into the industry segments you define, with a confidence score and the signals it used. Click the classification, see the source: the company description from Apollo, the way the prospect referred to their business on the call, the named competitors.',
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    sub: 'Build your catalogue with the columns you choose. CHRM recommends the right items per deal, with match scores and suggested pricing, grounded in what the prospect actually said.',
    meta: [
    ...
    prose: [
      ...
      'CHRM solves it by reading the conversation. Buyer says they need single sign-on, three-region deployment, and SOC 2 reporting? CHRM recommends the tier that includes them, scores the match, and proposes pricing before the rep has to flip to the price book.',
      { h: 'Catalogue, your way.' },
      'Build the catalogue with the columns that matter to you. Name, SKU, tier, pricing, included modules, eligibility rules, anything. CHRM reads the table and uses it as ground truth.',
      { h: 'Configurable products supported.' },
      'For multi-component or configurable products, CHRM recommends the configuration, not just the package. Each component recommendation comes with the buyer signal that motivated it.',
    ],
    bullets: [
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    prose: [
      'Sales playbooks are written once and revised never. The reason is simple: the data needed to revise them lives in conversations nobody logged. With CHRM, every conversation is structured. So is the outcome. The playbook stops being a Notion doc and becomes a system.',
      ...
      'When a hypothesis is strong enough to test, CHRM proposes a controlled experiment: a fraction of new deals get the proposed change, the rest run the existing playbook. Real impact is measured on real deals.',
      { h: 'Nothing auto-promotes.' },
      'No process change auto-applies to your team&rsquo;s workflow. CHRM proposes; a manager approves. The output is a clean recommendation with the evidence trail attached. Not a black-box optimiser.',
    ],
    bullets: [
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    h: 'Where CHRM <em>gets its data.</em>',
    ...
    prose: [
      ...
      'CHRM has its own notetaker built in. It auto-joins every booked call on your team&rsquo;s calendar — Zoom, Meet, Teams, phone — and produces a transcript without anyone installing a fourth tool. If your team already uses Fireflies, Fathom, or Otter, CHRM ingests their output alongside its own. They&rsquo;re one source among several, not a dependency.',
      { h: 'Email is two-way.' },
      'Gmail and Outlook are both first-class. Threads in both directions are captured into the deal timeline, and CHRM-drafted replies send via the rep&rsquo;s authenticated address, so prospects see the rep&rsquo;s name, not a robot&rsquo;s.',
      ...
      'Every existing note and activity in the connected CRM is ingested on day one. Your legacy context isn&rsquo;t stranded outside CHRM.',
    ],
    bullets: [
      ['Built-in notetaker', 'CHRM auto-joins every booked call. No third-party tool to install.'],
